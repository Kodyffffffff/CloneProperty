...
import { isMultiple } from '../../utills'
import Image from '../../components/Image'
import Review from '../../components/Review'
import Map from '../../components/Map'
import Link from 'next/link'
const Property = ({ //distraption in array key to client
  title,
  location,
  propertyType,
  mainImage,
    ...
    <div className="container">
      ...
      <div className="section">
        <div className="information">
          ...
          <h2><b>{propertyType} hosteb by {host?.name}</b></h2>       {/* query name in host */}
          <h4> {bedrooms} bedrooms {isMultiple(bedrooms)} * {beds}
            bed {isMultiple(beds)}</h4>
          <hr />
...
export const getServerSideProps = async (pageContext) => {
  const pageSlug = pageContext.query.slug //query slug in sanity
